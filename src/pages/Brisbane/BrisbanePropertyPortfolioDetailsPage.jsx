...
import BrisbanePropertyFormPortfolio from './BrisbanePropertyFormPortfolio.jsx';

const BrisbanePropertyPortfolioDetailsPage = () => {
  ...
  useEffect(() => {
    const getThreeRandomProperties = () => {
        const otherProperties = BrisbanePropertiesDataPortfolio.filter(p => p.id !== propertyId);
            ...
            {suggestedProperties.length > 0 ? (
              suggestedProperties.map((p) => (
                <div key={p.id} className="image-item-container">
                  <Link to={`/portfolio/brisbane/properties/${p.id}`}>
...
export default BrisbanePropertyPortfolioDetailsPage;