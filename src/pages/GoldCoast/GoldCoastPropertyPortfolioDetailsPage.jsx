...
import GoldCoastPropertyFormPortfolio from './GoldCoastPropertyFormPortfolio.jsx';

const GoldCoastPropertyPortfolioDetailsPage = () => {
  ...
  useEffect(() => {
    const getThreeRandomProperties = () => {
        const otherProperties = GoldCoastPropertiesDataPortfolio.filter(p => p.id !== propertyId);
  ...
  return (
    <>
      ...
      <div>
        ...
        <GoldCoastPropertyFormPortfolio />
            ...
            {suggestedProperties.length > 0 ? (
              suggestedProperties.map((p) => (
                <div key={p.id} className="image-item-container">
                  <Link to={`/portfolio/gold-coast/properties/${p.id}`}>
...
export default GoldCoastPropertyPortfolioDetailsPage;