...
import IpswichPropertyFormPortfolio from './IpswichPropertyFormPortfolio.jsx';

const IpswichPropertyPortfolioDetailsPage = () => {
  ...
  useEffect(() => {
    try {
      const getThreeRandomProperties = () => {
        const otherProperties = IpswichPropertiesDataPortfolio.filter(p => p.id !== propertyId);
      ...
      console.error('Suggested Properties Error:', error.message);
      setError(true);
    }
  }, [propertyId, property]);
            ...
            {suggestedProperties.length > 0 ? (
              suggestedProperties.map((p) => (
                <div key={p.id} className="image-item-container">
                  <Link to={`/portfolio/ipswich/properties/${p.id}`}>
...
export default IpswichPropertyPortfolioDetailsPage;